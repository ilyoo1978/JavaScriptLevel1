...
function isBlock(new_unit) {
    let check = false;
    const new_unit_classes = new_unit.getAttribute('class').split(' ')
    if(new_unit_classes.includes('block-unit')){
        check = true;
    }
    return check;
}
...
function createBlocks() {
    if(gameIsRunning) {
        ...
        if (!block_cell_classes.includes('snake-unit') && !block_cell_classes.includes('food-unit') && !block_cell_classes.includes('block-unit')) {
            var classes = block_cell.getAttribute('class') + ' block-unit';
            block_cell.setAttribute('class', classes);
            if (blocks.length > 5) {
                clearBlock = blocks.shift();
                clearBlock = clearBlock.getAttribute('class').split(' ')
                let clearBlockClasses = clearBlock[0] + ' ' + clearBlock[1];
                document.getElementsByClassName(clearBlock[1])[0].setAttribute('class', clearBlockClasses);
